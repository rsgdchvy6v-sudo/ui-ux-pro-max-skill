...
export default function KioskPage() {
  ...
  return (
    <div className="max-w-xl mx-auto space-y-4">
      <h1 className="text-xl font-bold text-stone-900">Kiosk Simulator</h1>
      ...
      <div className="card p-5 space-y-3">
        <label className="block text-xs font-medium text-stone-600">Scan Emirates ID</label>
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          {DEMO_IDS.map((d) => (
            <button key={d.eid} className="pill bg-stone-100 text-stone-600 hover:bg-stone-200" onClick={() => setEmiratesId(d.eid)}>
              {d.label}
            </button>
          ))}
        </div>
